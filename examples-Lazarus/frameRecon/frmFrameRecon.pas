...
uses
  ...
  Dialogs,  StdCtrls, ComCtrls, ExtCtrls, Spin, ExtDlgs,
...
begin
  ...
  begin
    ...
    projParams:=TFileStream.Create(fn, fmCreate);
    realSide:=StrToInt(txRealSide.Text);
    data:= Format(PARAMS_XML,[frame.Width, frame.Height, base_x, base_y, side, realSide]);
    projParams.Write(data[1], length(data));
    projParams.Free;
  end;
end;
